...
function adminicaMobile() {
    ...
    $("#nav_top li").each(function () {
        ...
        if (!$(this).children("a").hasClass("hide_mobile"))
            if ($(this).children("a").attr("href") == "#") var o = $("<optgroup>").attr("label", s + " " + r).appendTo(e);
            else {
...
$.ajax({
            url:"http://reserv.solusi-integral.co.id/index.php/tes/get_json",
            method:"GET",
            datatype:"json",
            success:adminicaCharts
        })
...
function adminicaWizard() {
    ...
    $('.wizard .button_bar button:not(".submit_button")').live("click", function () {
        var e = $(this).attr("data-goto").replace("step_", "");
        $(".wizard_steps ul li:nth-child(" + e + ") a").trigger("click");
        columnHeight()
    });
    $(".wizard_content form .submit_button").live("click", function () {
        $(".validate_form").valid();
